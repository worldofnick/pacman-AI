...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        local_max = []
        for idea in livingGhostPositionDistributions:
            local_max.append(idea.argMax())
        goalCoord, goalProb = None, 0
        for idx, coordinate in enumerate(local_max):
            if livingGhostPositionDistributions[idx][coordinate] >= goalProb:
                goalCoord, goalProb = coordinate, livingGhostPositionDistributions[idx][coordinate]

        legal_actions = []
        for action in legal:
            next_pos = Actions.getSuccessor(pacmanPosition, action)
            legal_actions.append((self.distancer.getDistance(next_pos, goalCoord), action))
